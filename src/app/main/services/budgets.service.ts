 ...
 public getDateHistory( budget: Budget): Array<BudgetHistoryDate> {
    ...
    if ( budget.fechaDeFactDeTaller){
     ...
     if(budget.motivoDeModificacion02){
      ...
      if(budget.motivoDelRechazo){
        ...
        if ( fecha !== '---' ){
          const data: BudgetHistoryDate = {
              ...
              date: fecha,
              createBy:usuario,
              description:budget.motivoDelRechazo.name 
          }
        tempArray.push(data);
       }
      ...
      if(budget.fechaDeAprobacionORechazo){
        const fecha = this.getStringFromTimestamp(budget.fechaDeAprobacionORechazo.createdAt)
        const usuario = budget.fechaDeAprobacionORechazo.createdBy
        if( fecha !== '---'){
          const data: BudgetHistoryDate = {
            type:'Fecha de aprobación',
            date: fecha,
            createBy:usuario
          }
          tempArray.push(data);
