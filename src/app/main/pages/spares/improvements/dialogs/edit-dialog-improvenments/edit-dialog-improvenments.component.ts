...
import {
  ImprovementEntry,
  ImprovementPart,
} from 'src/app/main/models/improvenents.model';
...
export class EditDialogImprovenmentsComponent implements OnInit {
  ...
  createFormListParts(): void {
    this.editImprovenmentsForm = this.fb.group({
      ...
      component: [
        this.data.component ? this.data.component : null,
        Validators.required,
      ],
      comments: [this.data.comments ? this.data.comments : null],
      date: [
        this.data.date ? new Date(this.data.date['seconds'] * 1000) : null,
        Validators.required,
      ],
  ...
  addControl(part?: ImprovementPart): void {
    ...
    if (part) {
      group = this.fb.group({
        sparePart: [
          part.sparePart ? part.sparePart : null,
          Validators.required,
        ],
        quantity: [part.quantity ? part.quantity : null, Validators.required],
        currentPart: [
          part.currentPart ? part.currentPart : null,
          Validators.required,
        ],
        improvedPart: [
          part.improvedPart ? part.improvedPart : null,
          Validators.required,
        ],
    ...
    if (this.editImprovenmentsForm.invalid) {
      ...
      this.auth.user$
        .pipe(
          take(1),
          switchMap((user) => {
            return this.impService.editImprovementEntry(
              this.data.id,
              this.editImprovenmentsForm.value,
              user
            );
          })
        )
          ...
          if (batch) {
            batch
              ...
              .catch((err) => {
                this.loading.next(false);
                this.snackbar.open(
                  '🚨 Hubo un error guardando la edición!',
                  'Aceptar',
                  {
                    duration: 6000,
                  }
